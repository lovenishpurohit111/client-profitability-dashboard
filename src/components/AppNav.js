    ...
    <>
      <div style={{
        ...
        <div className="flex items-center gap-2">
          <div style={{width:20,height:20,borderRadius:5,background:`linear-gradient(135deg,rgb(${current.color}),rgba(${current.color},0.6))`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:11}}>
            {current.icon}
          </div>
        ...
        <div className="relative">
          <button onClick={() => setOpen(!open)}
            ...
            style={{background:`rgba(${other.color},0.08)`,border:`1px solid rgba(${other.color},0.25)`,color:`rgb(${other.color})`}}>
          ...
          {open && (
            <div style={{position:'absolute',top:36,right:0,width:300,background:'#0f172a',border:'1px solid #334155',borderRadius:12,padding:16,boxShadow:'0 20px 60px rgba(0,0,0,0.5)',zIndex:200}}>
              ...
              {Object.entries(APPS).map(([key, app]) => {
                ...
                return (
                  <div key={key}
                    ...
                    style={{background:isCurrent?`rgba(${app.color},0.08)`:'rgba(30,41,59,0.4)',border:`1px solid ${isCurrent?`rgba(${app.color},0.3)`:'#1e293b'}`,cursor:isCurrent?'default':'pointer'}}>
                    <div style={{width:36,height:36,borderRadius:9,flexShrink:0,background:`rgba(${app.color},0.12)`,border:`1px solid rgba(${app.color},0.25)`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:18}}>
                      {app.icon}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="text-white text-sm font-semibold">{app.short}</p>
                        {isCurrent && <span className="text-xs px-1.5 py-0.5 rounded font-mono" style={{background:`rgba(${app.color},0.15)`,color:`rgb(${app.color})`}}>Current</span>}
                      </div>
